# -*- coding: utf-8 -*-
"""Combined Models with confusion matrix.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ShicE124Oaz6NaznHu5btr9rdTk4KuDy
"""
...
X_csv = scaler.fit_transform(np.array(df1.iloc[:, :-1], dtype = float))

class_list = df1.iloc[:, -1]
label = LabelEncoder()
y_csv = label.fit_transform(class_list)

X_train_csv, X_test_csv, y_train_csv, y_test_csv = train_test_split(X_csv,y_csv, test_size = 0.33, random_state=42)

def trainModel(model, epochs, optimizer):
  batch_size = 128
  model.compile(optimizer=optimizer, loss='sparse_categorical_crossentropy', metrics='accuracy')
  model.fit(X_train_csv, y_train_csv, validation_data = (X_test_csv, y_test_csv), epochs=epochs,batch_size=batch_size)
  return model

model = Sequential([
    Dense(512, activation='relu', input_shape=(X_train_csv.shape[1], )),
    Dropout(0.2),
...
history_2 = trainModel(model=model, epochs=50, optimizer='adam')

meta_1 = history_1.predict(X_train_image)
meta_2 = history_2.predict(X_train_csv)
...
combined_output = Dense(10, activation='relu')(combined_input)
...
print(X_test_csv.shape)

X_test_csv = X_test_csv[:X_test_image.shape[0]]
print(X_test_image.shape)
print(X_test_csv.shape)

new_1 = history_1.predict(X_test_image)
new_2 = history_2.predict(X_test_csv)
